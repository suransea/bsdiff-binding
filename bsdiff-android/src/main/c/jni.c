...
Java_com_shsuco_android_bsdiff_BSDiff_nativeDiff(JNIEnv *env, jclass thiz, jstring old_path,
...
JNIEXPORT jint JNICALL
Java_com_shsuco_android_bsdiff_BSDiff_nativePatch(JNIEnv *env, jclass thiz, jstring old_path,
                                              jstring new_path, jstring patch_path,
                                              jobjectArray msg_out) {
    const char *old = (*env)->GetStringUTFChars(env, old_path, NULL);
    const char *new = (*env)->GetStringUTFChars(env, new_path, NULL);
